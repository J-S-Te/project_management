...
stateDiagram-v2
    [*] --> 待确认: 创建项目（手动清单一律待确认）
    [*] --> 待分配: 合同激活/拆解调整未命中拆解规则（特殊方法项同步进待复核）
...
| R10 | 拆解规则**只作用于系统生成的清单**（合同激活、拆解调整）：存在启用规则且未命中 → 自动到「待分配」，否则「待确认」。**手动创建的项目一律「待确认」**，必须走服务项拆解确认；自动放行的特殊方法项同步置 `tech_review_status=PENDING`（待复核不能被跳过） | 【事实】 |
